...
class BuffClient(BanyanBase):
    def __init__(self,refresh_time=0.5):
        ...
        self.set_subscriber_topic('plotting') # for triggering plotting event
        self.set_subscriber_topic('me too') # to check whether the server is connected
        ...
        self.publish_payload({'loop_time':self.refresh_time},'connected')# to ensure server that client is connected, sends this message
        try:
    ...
    def incoming_message_processing(self, topic, payload):
        ...
        if topic=='me too': # reception of this topic ensures that the server is connected and ready for requests
            print("server connected and up and running")
            self.publish_payload({'loop_time':self.refresh_time},'request') # the message payload for intiating the request for data
