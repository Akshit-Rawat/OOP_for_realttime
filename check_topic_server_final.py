...
class BuffServer(BanyanBase):
    ...
    def __init__(self):
        ...
        self.set_subscriber_topic('connected') # to check whether the client is connected
    ...
    def incoming_message_processing(self, topic, payload):
        ...
        if topic=='connected': # reception of this topic ensures that the client is connected 
            
            
            print("client connected")
            self.publish_payload({'loop_time':0.5},"me too") # this notifies the client that the server is connected and ready for further processes
    ...
    def datageneration_loop(self): # thisis the receive addition idle loop, and it serves the function of data generation
            ...
            if self.topic_: # when the state changes to true, whcih happens when topic reques is received by the server. the data generation loop gets initiated 
                (self.t,self.xt) = self.generate_next_sample(current_sample=(self.t,self.xt),tau=0.1,sigma=1) # the generate next sample function generates new samples each time the idle loop runs.
